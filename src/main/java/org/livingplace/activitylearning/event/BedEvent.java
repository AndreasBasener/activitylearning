...
public class BedEvent extends Event{
	...
	public BedEvent(SleepState state, long time)
	{
		this.state = state;
		this.time = time;
	}
	...
	public double distanceTo(IEvent event) {
		if(event == this)
			return 0;
		if(event instanceof BedEvent)
		{
			BedEvent s = (BedEvent) event;
			switch (state) {
			case WACH:
				switch (s.state) {
				case WACH:
	...
	public boolean equals(Object o)
	{
		if(o == null)
			return false;
		if(!(o instanceof BedEvent))
			return false;
		
		BedEvent data = (BedEvent) o;
